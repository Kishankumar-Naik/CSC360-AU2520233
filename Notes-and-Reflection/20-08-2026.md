...
We discussed how to maintain the course repository in a consistent way. Each class session should have its own Markdown file containing the class notes, questions and reflection together. The notes should explain the concepts covered in class, while the reflection records the main understanding from that session.
...
```mermaid
flowchart LR
    A["Triangle Vertices V1 V2 V3"] --> B["Calculate Bounding Box"]
    B --> C["Evaluate Edge Functions"]
    C --> D{"Inside Boundary?"}
    D -->|Yes| E["Fill Pixel"]
    D -->|No| F["Skip Pixel"]
```
